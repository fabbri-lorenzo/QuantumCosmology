...
\begin{eqopt}[darkred]\label{mode_dS}
    v_k(\tau) = \left[ C_1 \frac{e^{-ik\tau}}{\sqrt{2k}}\, \left(1-\frac{i}{k\tau}\right)+ C_2 \frac{e^{ik\tau}}{\sqrt{2k}} \,\left(1+\frac{i}{k\tau}\right) \right]
\end{eqopt}
...
The physical wavelength is defined as \textcolor{darkgreen}{$\lambda_{\text{phys}} = a \lambda$}. $\abs{k\tau} \propto (H)^{-1}/\lambda_{\text{phys}}$, 
imposing $\gg 1$ we find that \emph{background curvature (Hubble radius) is much grater than the physical wavelength of the mode $\Rightarrow$ at early times modes behave as they were in Minkowski spacetime}.
