...
For each model calculate $\epsilon_V, \eta_V$ and impose $\ll 1$. Then find $\phi(N)$ to get the field value for 60 e-foldings. Rewrite $\epsilon_V(N), \eta_V(N)$
...
\begin{mycolorbox}[pink!93!black]
    ...
    \end{eqopt}
    $N_{tot}=\int dN = \int_{\phi(end)}^{\phi(start)} \frac{d\phi}{\sqrt{2\epsilon}}$. $\phi(end)\sim \sqrt{2}M_P$ and $\phi(start)\equiv \phi(N_{tot})\sim 15M_P$.
    \begin{equation}
